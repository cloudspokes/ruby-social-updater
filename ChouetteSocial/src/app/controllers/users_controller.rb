...
class UsersController < ApplicationController
  ...
  def do_chatter_feeds
    ...
    ct = ApplicationHelper::chatter_client(token_refresh)
    ...
    query = "SELECT Id, CreatedDate, CreatedBy.Name, CreatedBy.Id, FeedPost.Id, FeedPost.Body from UserFeed where parentid='#{profile_id}' order by CreatedDate Desc limit 5"
  ...
  def do_twitter_feeds
    ...
    @twitter_feeds = tc.home_timeline({:count => 5})
  end
  ...
  def do_linked_in_feeds
    ...
    network_feeds = lnk_client.network_updates({:count => 5, :scope => 'self'})
